...
for (const i in EMOTICON_DATA) {
  for (const j in EMOTICON_DATA[i].list) {
    if (EMOTICON_DATA[i].list[j].name) {
      EMOTICON_DATA[i].list[j].title = EMOTICON_DATA[i].list[j].name
    } else {
      EMOTICON_DATA[i].list[j].title = EMOTICON_DATA[i].list[j].title.replace(
        /(^\S+ )|(^\S+-)|(^\S+_)|呜呜/,
        ''
      )
    }
